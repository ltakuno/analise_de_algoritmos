...
\li \> \For $j = i+1$ \To $n$ {\blue \textbf{faça}}
...
\begin{slide}
  \stitle{Contagem de inversões} 
  Análise do algoritmo $\proc{conta-e-intercala}$:
  
\itemtrig\ linhas 1-4 : $O(n)$   

\vspace{-1cm}\itemtrig\ linhas 5-7 : $O(1)$   
  
\vspace{-1cm}\itemtrig\ linhas 8-14 : $O(n)$   

\vspace{-1cm}\itemtrig\ linha 15 : $O(1)$   
...
\begin{slide}
  ...
  Análise do algoritmo $\proc{conta-e-ordena}$:
  
  $T(n) = 2 T(n/2) + n$

definida para $n$ potência de 2.

Prova utilizando a técnica da \textbf{árvore de recorrência}.

$T(n) = \left\{ 
\begin{array}{ll}
1 & \textrm{se } n=1 \\\\
2T(n/2) + n & \textrm{se } n\geq 2, n\textrm{ potência de }2 \\
\end{array}
\right.$
\end{slide}
...
$\begin{array}{ll}
T(n) & = 2T(n/2) + n\\
     & = 2T(2T(n/2^2) + n/2) + n = 2^2T(n/2^2) + 2n\\
     & = 2^2T(2T(n/2^3) + n/2^2) + 2n = 2^3T(n/2^3) + 3n\\
     & = 2^3T(2T(n/2^4) + n/2^3) + 3n = 2^4T(n/2^4) + 4n\\
\end{array}$
...
onde $k = \lg n$. Disso concluimos que $T(n) = n + n\lg n$  
...
Prova utilizando o \textbf{método da substituição}
...
\textbf{Hipótese de indução:} $T(n) = n + n \lg n$  para $m < n$       


\textbf{Passo de indução: } $n \geq 2$
...
15                       & 20                      & 12                      & 18                      \\
10                       & 19                      & 11                      & 17                     \\ \hline
\multicolumn{1}{|c}{9} & \multicolumn{1}{c}{6} & \multicolumn{1}{c}{5} & \multicolumn{1}{c|}{16} \\ \hline
8                      & 4                     & 3                    & 14                      \\
7                      & 1                    & 2                     & 13                     
\end{tabular}
...
12                       & 20                      & 15                      & 18                      \\
11                       & 19                      & 10                      & 17                      \\ \hline
\multicolumn{1}{|c}{5} & \multicolumn{1}{c}{6} & \multicolumn{1}{c}{9} & \multicolumn{1}{c|}{16} \\ \hline
3                     & 4                     & 8                     & 14                      \\
2                      & 1                     & 7                     & 13                     
\end{tabular}
...
\itemcirc\ Elementos de $A$ são menores ou iguais a $m$.

\vspace{-1cm}\itemcirc\ Elementos de $B$ são maiores ou iguais a $m$.
...
\begin{slide}
  \stitle{seleção do k-ésimo mínimo} 
\itemtrig\  A lista $M$ das medianas contém no máximo $n/5$ elementos então 

\vspace{-1cm} \hspace{2cm}\itemtrig\ linha 5: $T(n/5)$ no máximo.

\itemtrig\  As chamada recursivas de $\proc{seleção}(k, L_1)$ e $\proc{seleção}(k - |L_1| - |L2|, L_3)$ requerem tempo máximo $T(3n/4)$. 

\itemtrig\  As demais linhas requerem tempo $O(n)$.
