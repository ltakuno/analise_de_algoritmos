...
\begin{slide}
 ...
 Gerar todas as sequências possíveis de 3 dígitos com os dígitos 1,2 e 3 armazenados no vetor $v[0 \cdot\cdot\; n-1]$, onde $n=3$
...
\vspace{-1cm}\hspace{3.7cm}inteiro $n$ o tamanho do vetor $v$

  \textbf{\darkgreen saída:} apresenta todas as sequências de $n$ elemntos de $v$   
...
\begin{slide}
  \stitle{Sequências}    
\begin{center}
\includegraphics[width=1.1\textwidth]{fig01}
\end{center}
\end{slide}
...
\begin{slide}
  ...
  Suponha que agora queremos gerar todas as combinações de tamanho 3 utilizando os dígitos $v = \{1, 2, 3, 4\}$. Nesta caso, temos que definir $s[0 \cdot\cdot\; m-1]$, com $m=3$, e o vetor $v[0 \cdot\cdot\; n-1]$, com $n=4$.
...
\vspace{-1cm}\hspace{3.7cm}que já foram inseridos no conjunto solução. 
  ...
  \textbf{\darkgreen saída:} apresenta a permutação dos elementos do vetor $v$ 
\end{slide}
