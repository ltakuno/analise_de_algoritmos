...
\begin{slide}
  \stitle{Oito rainhas} 
\begin{codebox}
\Procname{$\proc{coloca\_coluna}(i)$}
\li \textbf{\blue repita}
\li  \> selecione uma posição
\li  \> \textbf{\blue se} "rainha está salva" \textbf{\blue então}
\li  \> \>posicione a rainha
\li  \> \>\textbf{\blue se} i < 7 \textbf{\blue então}
\li  \> \> \>$\proc{coloca\_coluna}(i+1)$
\li  \> \> \>\textbf{\blue se} "sem sucesso" \textbf{\blue então}
\li  \> \> \> \>remova rainha
\li \textbf{\blue até que} "ocorra sucesso" ou "não há mais pos"
\end{codebox}  
\end{slide}
...
\itemtrig\ Um vetor $x$ tal que $x[i] = j$ onde $j$ é a posição da rainha 

\vspace{-1cm}\hspace{1cm}na coluna $i$, $0 \leq i \leq 7$ 

\vspace{-1cm}\itemtrig\ Um vetor a tal que:
  
\hspace{2cm}$
   a[j] = \quad \left\{ \begin{array}{ll} 
                                 1, & \mbox{se a linha j encontra-se livre} \\
                                 0, & \mbox{caso contrário}
                     \end{array} \right.
$

\hspace{2cm}$0 \leq j \leq 7$ 
\end{slide}

\begin{slide}
  \stitle{Oito rainhas} 

\itemtrig\ Um vetor b tal que:

\hspace{2cm}$
   b[i+j] = \quad \left\{ \begin{array}{ll} 
                                 1, & \mbox{se a diagonal ``/'' encontra-se livre} \\
                                 0, & \mbox{caso contrário}
                     \end{array} \right.
$

\hspace{2cm}$0 \leq i+j \leq 14$  

\itemtrig\ Um vetor c tal que 

\hspace{2cm}$
   c[i-j+7] = \quad \left\{ \begin{array}{ll} 
                                 1, & \mbox{se a diagonal ``$\backslash$''  encontra-se livre} \\
                                 0, & \mbox{caso contrário}
                     \end{array} \right.
$

\hspace{2cm}$0 \leq i-j+7 \leq 14$
\end{slide}

\begin{slide}
  \stitle{Oito rainhas} 
Executar o comando ``posicionar rainha'' é o mesmo que executar
...
Para testar se a rainha está salva na linha j e na coluna i basta testar se:
...
\begin{verbatim}
void posicione_rainha(int i, int j){
   x[i] = j;
   a[j] = 0;   b[i+j] = 0;   c[i-j+7] = 0;
}

void remova_rainha(int i, int j){
   a[j] = 1;   b[i+j] = 1;   c[i-j+7] = 1;
}

bool rainha_salva(int i, int j){
...
\begin{slide}
  \begin{verbatim}
void coloca_rainha(int i, int *ok){
   int j = 0;
   do{
      *ok = 0;
      if (rainha_salva(i, j)){
         posicione_rainha(i, j);
         if (i < 7){
            coloca_rainha(i+1, ok);
            if (!*ok) remove_rainha(i, j);
         }
         else *ok = 1; 
      }
      j++;
