...
\begin{slide}
  \stitle{Limite inferior para ordenação}  
\begin{theorem}
Qualquer árvore de decisão que ordena $n$ elementos tem altura $\Omega(n \log n)$.
\end{theorem}  
\textbf{Dem.:} Considere uma árvore de decisão de altura $h$, que ordena $n$ elementos. Como existem $n!$ permutações distintas de $n$ elementos, cada uma apresentando uma ordenação desses elementos, então a árvore de decisão deve ter $n!$ folhas. 
\end{slide}
 
\begin{slide}
  \stitle{Limite inferior para ordenação}  

Também como uma árvore binária de altura $h$ tem no máximo $2^h$ folhas, então a árvore de decisão tem no máximo $2^h$ folhas.

Assim,
	
	$$n! \leq \textrm{número de folhas} \leq 2^h$$
...
$
n! = \sqrt{2 \pi n} \displaystyle \left(\frac{n}{e} \right)^n > \displaystyle \left(\frac{n}{e} \right)^n
$  
...
$
\begin{array}{l}
h \geq \log(n!) \Rightarrow  h > \log \displaystyle \left(\frac{n}{e} \right)^n\\
= n \log \frac{n}{e} \\
